...
public sealed class OpenXmlExcelMarkupMergeServiceTests
{
    [Fact(DisplayName = "Merge restores row fill and comment from newest old workbook")]
    [Trait("Category", "Unit")]
    public void MergeRestoresRowFillAndCommentFromNewestOldWorkbook()
    {
        ...
        try
        {
            ...
            new OpenXmlWorkbookFormatter().Format(newStream, new Dictionary<ExcelSheetName, ExcelSheetLayout>
            {
                [new ExcelSheetName("Sheet1")] = new ExcelSheetLayout(new ExcelSheetName("Sheet1"))
            });
            var service = CreateService(tempDirectory);

            var summary = service.Merge(newStream, new Dictionary<ExcelSheetName, ExcelSheetLayout>
            {
                [new ExcelSheetName("Sheet1")] = new ExcelSheetLayout(new ExcelSheetName("Sheet1"))
            });
            ...
            worksheet.Descendants<Cell>().Select(cell => cell.CellReference?.Value).Should().ContainInOrder("A5", "B5", "C5", "D5", "E5");
            ...
            var commentFillId = cellFormats[(int)commentCell.StyleIndex!.Value].FillId!.Value;
            fills[(int)commentFillId].PatternFill!.ForegroundColor!.Rgb!.Value.Should().Be("F59E0B");
    ...
    [Fact(DisplayName = "Merge restores manual fill even when previous workbook stores it under a low fill id")]
    [Trait("Category", "Unit")]
    public void MergeRestoresManualFillWhenPreviousWorkbookUsesLowFillId()
    {
        var tempDirectory = Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString("N"));
        Directory.CreateDirectory(tempDirectory);
        var oldWorkbookPath = Path.Combine(tempDirectory, "old-report.xlsx");
        ...
        try
        {
            ...
            ApplyManualFillWithLegacyPalette(oldWorkbookPath, "Sheet1", "A5", "E5");
            File.SetLastWriteTimeUtc(oldWorkbookPath, DateTime.UtcNow);
            ...
            _ = service.Merge(newStream, new Dictionary<ExcelSheetName, ExcelSheetLayout>
            {
                [new ExcelSheetName("Sheet1")] = new ExcelSheetLayout(new ExcelSheetName("Sheet1"))
            });
            ...
            fills[(int)fillId].PatternFill!.ForegroundColor!.Rgb!.Value.Should().Be("FFFF0000");
        }
        finally
        {
            Directory.Delete(tempDirectory, recursive: true);
    ...
    [Fact(DisplayName = "Merge does not report merged rows when old workbook has no manual markup to restore")]
    [Trait("Category", "Unit")]
    public void MergeWhenOldWorkbookHasNoManualMarkupDoesNotReportMergedRows()
    {
        var tempDirectory = Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString("N"));
        Directory.CreateDirectory(tempDirectory);
        var oldWorkbookPath = Path.Combine(tempDirectory, "old-report.xlsx");
    ...
    private static OpenXmlExcelMarkupMergeService CreateService(string? oldReportsPath = null) =>
    ...
    private static void ApplyManualFillWithLegacyPalette(string workbookPath, string sheetName, string startCell, string endCell)
    {
        using var document = SpreadsheetDocument.Open(workbookPath, true);
        var workbookPart = document.WorkbookPart ?? throw new InvalidOperationException("Workbook part is missing.");
        ...
        var sheet = workbook.Sheets!.OfType<Sheet>().Single(candidate => candidate.Name == sheetName);
